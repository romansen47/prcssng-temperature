...
public interface IBall {
	...
	default public Ball[] Collision(Ball[] Balls, int LevelOfCorrectness) {
		...
		if (Balls.length == 2) {
			...
			if (LevelOfCorrectness > 0) {
				...
				Balls[0].setPosition(
						ScalarMultiplication(0.5, AdditionOfVectors(TmpBalls1[0].getPosition(), tmp2.getPosition())));
				Balls[Balls.length - 1].setPosition(ScalarMultiplication(0.5,
						AdditionOfVectors(TmpBalls2[TmpBalls2.length - 1].getPosition(), tmp1.getPosition())));
				for (int i = 1; i < Balls.length - 1; i++) {
					Balls[i].setPosition(ScalarMultiplication(0.5,
	...
	default public Ball[] Interaction(Ball A, Ball B) {
		...
		if (Abs(Tmp[0]) <= A.getRadius() + B.getRadius() && Abs(Tmp[1]) <= A.getRadius() + B.getRadius()) {
			double Condition = MagnitudeOfVector(AdditionOfVectors(
					AdditionOfVectors(ATmp.getPosition(), ATmp.getVelocity()),
					AdditionOfVectors(ReversalOfVector(BTmp.getPosition()), ReversalOfVector(BTmp.getVelocity()))));
			if (Condition <= A.getRadius() + B.getRadius()) {
				...
				PointOfCollision = AdditionOfVectors(A.getPosition(),
						ScalarMultiplication(A.getRadius(), UnitVector(connection)));
				A.setPosition(
						ScalarMultiplication(0.5, AdditionOfVectors(A.getPosition(), AdditionOfVectors(PointOfCollision,
								ScalarMultiplication(-A.getRadius() - 1, UnitVector(connection))))));
				B.setPosition(
						ScalarMultiplication(0.5, AdditionOfVectors(B.getPosition(), AdditionOfVectors(PointOfCollision,
								ScalarMultiplication(B.getRadius() + 1, UnitVector(connection))))));
				...
				A.setVelocity(AdditionOfVectors(A.getVelocity(),
						ScalarMultiplication(2 * B.getMass() / (A.getMass() + B.getMass()),
								AdditionOfVectors(projectedVelocityB, ReversalOfVector(projectedVelocityA)))));
				B.setVelocity(AdditionOfVectors(B.getVelocity(),
						ScalarMultiplication(2 * A.getMass() / (A.getMass() + B.getMass()),
								AdditionOfVectors(projectedVelocityA, ReversalOfVector(projectedVelocityB)))));
			}
		}
