...
public class Ball {
    ...
    public Ball Prediction(){
		  ...
		  NewVelocity[1]=Velocity[1]-Config.getAirDrag()*Functions.mathOperator.SignumFunction(Velocity[1])+Config.getGravitationalConstant();
    ...
    public void move() {
       ...
       if (getPosition()[1]+Velocity[1]>=Config.getBottom()-getRadius()){
         ...
         if (Functions.mathOperator.Abs(Velocity[1])<Config.getMinVerVel()) Velocity[1]=0;
         else {
           if (Velocity[1]>=0) Velocity[1]=-Config.getLossOfEnergyAtTheWallByDrag()*Velocity[1];
           else Velocity[1] = Config.getLossOfEnergyAtTheWallByDrag()*Velocity[1]
        		   				-Config.getAirDrag()*Functions.mathOperator.SignumFunction(Velocity[1])
        		   				+getMass()*Config.getGravitationalConstant();
         }
       }
       else Velocity[1] = Velocity[1]-Config.getAirDrag()*Functions.mathOperator.SignumFunction(Velocity[1])
